...
total.fechas <- seq(fecha.inicio, fecha.final, by = "month") #day
...
dataset.btc <- dataset.btc[!(month(dataset.btc$Date) == 2 & day(dataset.btc$Date) == 29), ] #Se eliminan días biciestos
summary(dataset.btc)

#dataset.btc <- dataset.btc %>% group_by(Date = floor_date(Date, "month")) %>% summarise(Close = mean(Close))
...
dia.inicio <- which(dataset.btc$Date == "2014-10-01") # Obtenemos el número de día. 2014-09-17
dia.inicio
...
## **Construcción de modelado básico**
...
## **Construcción de modelado avanzados**
...
## **Evaluación de modelados avanzados**
...
<div style="text-align: justify"> 
Seleccionamos el mejor modelo y realizamos una nueva predicción utilizando toda la serie de tiempo. En este caso utilizaremos Arima calibrado con el periodo 6 </div>
...
dygraph(serie.final, main = "Predicción Bitcoin para los próximos 6 meses", xlab = "Año", width = "100%") %>%
  dyAxis("y", label = "Precio($)") %>%
...
<div style="text-align: justify"> 
Seguidamente se presentará la descripción de los datos para Ethereum
...
dataset.eth <- dataset.eth[!(month(dataset.eth$Date) == 2 & day(dataset.eth$Date) == 29), ] #Se eliminan días biciestos
summary(dataset.eth)

#dataset.eth <- dataset.eth %>% group_by(Date = floor_date(Date, "month")) %>% summarise(Close = mean(Close))
```
...
dia.inicio <- which(dataset.eth$Date == "2015-09-01") # Obtenemos el número de día. 2015-08-07
dia.inicio
...
dygraph(serie.final, main = "Predicción Ethereum para los próximos 6 meses", xlab = "Año", width = "100%") %>%
  dyAxis("y", label = "Precio($)") %>%
  dySeries(c("prediccion.LimInf", "prediccion.pred.arima.6", "prediccion.LimSup"), label = "Predicción") %>%
  dyLegend(show = "follow") %>% 
  dyUnzoom() %>% 
